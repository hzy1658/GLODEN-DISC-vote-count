...
# 定义 URL
MY1PICK_URL = "https://cdn.fandomchart.com/web/rank/json/1.MO.1.json"
MUBEAT_URL = "https://cdn.fandomchart.com/web/rank/json/1.MB.1.json"
FANDOM_URL = "https://cdn.fandomchart.com/web/rank/json/1.FC.1.EN.json"

# 定义 CSV 文件名
CSV_FILE = 'GLODEN-DISC-male-vote_counts.csv'
...
while True:
    ...
    print("\nDetailed Votes:")
    print(
        f"{'Name':<20} {'Total Votes':<15} {'My1Pick Votes':<15} {'Mubeat Votes':<15} {'Fandom Votes':<15} {'Timestamp':<20} "
        f"{'My1Pick Diff':<15} {'Mubeat Diff':<15} {'Fandom Diff':<15} {'Total Diff':<15}")
    for name in data_to_write:
        print(f"{name[0]:<20} {name[1]:<15} {name[2]:<15} {name[3]*2:<15} {name[4]:<15} {name[5]:<20} "
